...
| 2026-08-29 | Added Upstash Redis (`@csweek/cache`) for TTL read-through caching (web home/events/detail 30s, admin overview 15s) and fixed-window rate limiting on the anonymous registration action (10/event/IP/15min + 50/IP/hour, HTTP 429). Fails open when Redis is unavailable. Capacity check stays on live Postgres | Vercel serverless (no self-hosted Redis); the public registration action is the only unauthenticated write touching the service-role key, so it is the top abuse target. Caching cuts repeated count/list queries during the registration burst. Fail-open keeps an infra outage from blocking page loads or signups; TTLs are so short that stale counts are acceptable and mutations bust keys best-effort |
